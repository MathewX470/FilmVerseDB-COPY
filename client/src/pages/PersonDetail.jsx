...
const PersonDetail = () => {
  ...
  useEffect(() => {
    // Guard against undefined personId
    if (!personId) {
    ...
    const getPerson = async () => {
      ...
      else toast.error("Failed to load person details");
    };

    getPerson();
  }, [personId]);

  // Only render if person data is available
  return (
    <>
      <Toolbar />
      {person ? (
                ...
                <Box
                  sx={{
                    ...
                    backgroundImage: `url(${tmdbConfigs.posterPath(
                      person.profile_path || ""
                    )})`, // Fallback for missing profile_path
                  }}
                />
              </Box>
              <Box
        ...
        <Typography>Loading person details...</Typography> // Fallback UI
      )}
    </>
  );
};
